...
static int skw_get_key(struct wiphy *wiphy,struct net_device *netdev,int link_id,
...
{
	skw_dbg("dev: %s, key_index: %d, pairwise: %d, mac: %pM\n",
		netdev_name(wdev), key_index, pairwise, mac_addr);
	return 0;
#endif	
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0) || 1
...
{
#else
static int skw_add_key(struct wiphy *wiphy, struct net_device *dev, int key_index, 
		       u8 pairwise, const u8 *mac_addr, struct key_params *params)
{
#endif
	const u8 *mac;
	int ret, key_type;
...
#else
	return skw_add_key(wiphy, wdev->netdev, link_id, 
			key_idx, pairwise, addr, params);
#endif
}
...
/* for WEP keys */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0) || 1
static int skw_set_default_key(struct wiphy *wiphy, struct wireless_dev *wdev,
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0) || 1
static int __skw_set_default_key(struct wiphy *wiphy, struct wireless_dev *wdev,
			       int link_id, u8 key_idx, bool unicast,
			       bool multicast)
{
	return skw_set_default_key(wiphy, wdev, 	link_id,
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0) || 1
static int skw_set_mac_acl(struct wiphy *wiphy, struct wireless_dev *wdev,
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0) || 1
static int skw_set_mib(struct wiphy *wiphy, struct wireless_dev *wdev)
{
	struct net_device *dev = wdev->netdev;
#else
static int skw_set_mib(struct wiphy *wiphy, struct net_device *dev)
{
#endif
	int ret = 0;
	u16 *plen;
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0) || 1
static int skw_start_ap(struct wiphy *wiphy, struct wireless_dev *wdev,
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0) || 1
static int skw_sap_del_sta(struct wiphy *wiphy, struct wireless_dev *wdev,
...
static int skw_stop_ap(struct wiphy *wiphy,
		struct wireless_dev *wdev unsigned int link_id SKW_NULL)
{
	int ret;
	struct skw_iface *iface = netdev_priv(dev);
...
static int skw_change_beacon(struct wiphy *wiphy, struct wireless_dev *wdev
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0) || 1
static int skw_change_station(struct wiphy *wiphy, struct wireless_dev *wdev,	
		const u8 *mac,
...
int skw_cmd_del_sta(struct wiphy *wiphy, struct wireless_dev *wdev
...
int skw_delete_station(struct wiphy *wiphy, struct wireless_dev *wdev
...
static void skw_fix_compatiblity_issues(struct wiphy *wiphy,
		struct skw_iface *iface, struct cfg80211_auth_request *req)
{
	const u8 *he_oper_ie;
	struct skw_he_oper_elem *he_oper;
	struct skw_core *skw = iface->skw;
	//On 5G channel, BSS color set 0, max bw 40

	rcu_read_lock();

	he_oper_ie = skw_bss_get_ext_ie(req->bss, SKW_WLAN_EID_EXT_HE_OPERATION);
	if (he_oper_ie) {
		he_oper = (struct skw_he_oper_elem *)(he_oper_ie + 3);
		skw_dbg("Get HE OPER bss_color:%d bss_color_disabled:%d\n",
			he_oper->bss_color, he_oper->bss_color_disabled);
	}

	skw_dbg("band:%d chip->fw_bw_capa:0x%x\n", req->bss->channel->band, skw->fw.fw_bw_capa);

	if (req->bss->channel->band == NL80211_BAND_5GHZ &&
		!(skw->fw.fw_bw_capa &
		    ...
		    struct cfg80211_auth_request *req)
{
	...
	skw_fix_compatiblity_issues(wiphy, iface, req);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0) || 1
static int  skw_join_ibss(struct wiphy *wiphy, struct wireless_dev *wdev, 
